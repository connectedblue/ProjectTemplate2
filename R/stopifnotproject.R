# This file contains internal helper functions to determine whether a given path is a ProjectTemplate2directory
# or not.
...
# files that determine whether a directory is a ProjectTemplate2project
.mandatory.files <- c("config/global.dcf", "cache", "data")

# Test whether a given path is a ProjectTemplate2project
        ...
        if (!is.ProjectTemplate) {
                ...
                message(
                        paste0(c(paste0(directory, basename(path),
                                        " is not a ProjectTemplate2directory"),
                                 additional_message),
                               sep = "\n")
                )
...
.stopifproject <- function(additional_message="", path=getwd()) {
        ...
        if (is.ProjectTemplate) {
                ...
                message(
                        paste0(c(paste0(directory, basename(path),
                                        " is a ProjectTemplate2directory"),
                                 additional_message),
                               collapse = "\n")
                )
